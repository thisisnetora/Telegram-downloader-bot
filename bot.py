...
# The server's own output goes here instead of /dev/null — without it a crash at
# boot (missing lib, bad build) is invisible and all you get is "exited early".
POT_LOG_FILE = Path(tempfile.gettempdir()) / "bgutil-pot-server.log"
_pot_proc = None
_pot_ready = False


def _pot_log_tail() -> str:
    try:
        return POT_LOG_FILE.read_text(encoding="utf-8", errors="replace")[-2000:].strip()
    except OSError:
        return ""
...
def start_pot_server() -> None:
    ...
    try:
        ...
        log_fh = open(POT_LOG_FILE, "w", encoding="utf-8", errors="replace")
    ...
    for _ in range(40):  # up to ~20s for the server to bind
        ...
        if _pot_proc.poll() is not None:
            logger.error(
                "bgutil POT server exited early (code %s). Output:\n%s",
                _pot_proc.returncode, _pot_log_tail() or "(no output)",
            )
            _pot_proc = None
            return
        time.sleep(0.5)
    logger.error(
        "bgutil POT server did not become ready in time. Output so far:\n%s",
        _pot_log_tail() or "(no output)",
    )
